...
##### Ответ преподователя. Папку target коммитить нельзя, она автогенерируемая мавеном и в ней хранятся автогенерируемые файлы при запусках команд системы сборки, ничего полезного для сохранения в репозитории в ней нет; её надо удалить из системы контроля версий через git rm -r --cached и добавить в гитигнор, находящийся в корневой папке репозитория
## Описание
